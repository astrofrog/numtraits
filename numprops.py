...
class NumericalProperty(object):
    ...
    def __init__(self, name, domain=None, shape=None, default=None):
        ...
        self.name = name
        self.domain = domain
        self.ndim = len(shape) if shape is not None and not isinstance(shape, str) else None
        self.shape = shape
    ...
    def _validate_array(self, instance, value):
        ...
        if not isinstance(value, np.ndarray) or value.ndim != self.ndim and self.ndim is not None:
            if self.ndim == 1:
